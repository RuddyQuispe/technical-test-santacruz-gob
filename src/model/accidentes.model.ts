...
export class AccidentesModel {
    ...
    public findById = async (id: number): Promise<
        ...
        try {
            let accident = await this.connectionSQL.executeSQL(
                `select a.acc_id, to_char(a.acc_fecha, 'YYYY-MM-DD') as acc_fecha, a.acc_severidad, a.acc_emp_id, e.emp_nombre, e.emp_apellido, a.acc_sector_id, s.sector_desc, a.acc_ta_id, ta.ta_desc  
                from accidentes a, empleados e, sectores s, tipos_accidentes ta
                where a.acc_emp_id=e.emp_id and e.emp_sector_id=s.sector_id and ta.ta_id=a.acc_ta_id and a.acc_id=2;`);
            return accident.rows[0];
        } catch (error) {
            console.log(`Error into AccidentesModel -> findById() ${error}`);
            return undefined;
        }
    }
    ...
    public update = async (id: number, fecha: string, severidad: string, idEmpleado: number, idSector: number, idTipoAccidente: number): Promise<boolean> => {
        try {
            ...
            console.log(`Error into AccidentesModel -> update() ${error}`);
            return false;
        }
    }
